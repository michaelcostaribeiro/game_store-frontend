...
const Cart = () => {
    const { data: cartItems, loading, error } = useFetch({ endpoint: 'getCart/', auth: true })

    if (loading) return <LoadingScreen />
    if (error) return <ErrorScreen />

    return <div className='min-h-[70vh] my-5 flex flex-col justify-center items-center gap-2 px-20 xl:px-100'>

        {cartItems ? <>
            <h1 className='font-semibold text-left w-full text-2xl'>Itens no carrinho:</h1>
            <div className='flex flex-col items-center gap-2 w-full'>
                {cartItems.map((item) => {
                    return <div key={item.id} className=' bg-secondary h-40 flex rounded-xl overflow-hidden w-full'>
